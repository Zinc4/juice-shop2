...
const validatePreconditions_1 = __importDefault(
  require("./lib/startup/validatePreconditions")
);
const cleanupFtpFolder_1 = __importDefault(
  require("./lib/startup/cleanupFtpFolder")
);
const validateConfig_1 = __importDefault(
  require("./lib/startup/validateConfig")
);
const restoreOverwrittenFilesWithOriginals_1 = __importDefault(
  require("./lib/startup/restoreOverwrittenFilesWithOriginals")
);
const registerWebsocketEvents_1 = __importDefault(
  require("./lib/startup/registerWebsocketEvents")
);
const customizeApplication_1 = __importDefault(
  require("./lib/startup/customizeApplication")
);
const customizeEasterEgg_1 = __importDefault(
  require("./lib/startup/customizeEasterEgg")
); // vuln-code-snippet hide-line
const authenticatedUsers_1 = __importDefault(
  require("./routes/authenticatedUsers")
);
...
void collectDurationPromise(
  ...
  validatePreconditions_1.default
)();
...
(0, restoreOverwrittenFilesWithOriginals_1.default)()
  .then(() => {
    /* Locals */
    ...
    // fs.watchFile("logs/access.log", (curr, prev) => {
    ...
    app.use((req, res, next) => {
      res.append(
        "X-Recruiting",
        config_1.default.get("application.securityTxt.hiring")
      );
      next();
    });
    ...
    app.use(
      ...
      securityTxt({
        ...
        acknowledgements: config_1.default.get(
          "application.securityTxt.acknowledgements"
        ),
        "Preferred-Languages": [
          ...new Set(locales.map((locale) => locale.key.substr(0, 2))),
        ].join(", "),
        hiring: config_1.default.get("application.securityTxt.hiring"),
        csaf:
          config_1.default.get("server.baseUrl") +
          config_1.default.get("application.securityTxt.csaf"),
      ...
      res.end = function () {
        if (arguments.length) {
          ...
          arguments[0] = arguments[0].replace(
            /a href="([^"]+?)"/gi,
            function (matchString, matchedUrl) {
    ...
    // Gunakan format kustom
    const customFormat = `
      :method :url :status :response-time ms - :res[content-length]
      | Headers: :headers 
      | Body: :body`;

    // Middleware Morgan untuk menyimpan log ke file
    app.use(
      morgan(customFormat, {
    ...
    app.use((req, res, next) => {
      try {
        ...
        res.send = function (body) {
          ...
          axios
            .post("http://localhost:8088/api/logs", {
              method: req.method,
              url: req.url,
              headers: req.headers,
              body: req.body,
    ...
    // Middleware Morgan untuk log ke console (format dev untuk debugging)
    app.use(
      morgan("dev", {
        skip: (req, res) => req.url === "/favicon.ico", // Lewati favicon.ico
      })
    );
    ...
    const autoModels = [
      ...
      { name: "Recycle", exclude: [], model: recycle_1.RecycleModel },
      {
        ...
        exclude: [],
        model: securityQuestion_1.SecurityQuestionModel,
      },
      {
        name: "SecurityAnswer",
        exclude: [],
        model: securityAnswer_1.SecurityAnswerModel,
      },
      { name: "Address", exclude: [], model: address_1.AddressModel },
      {
        name: "PrivacyRequest",
        exclude: [],
        model: privacyRequests_1.PrivacyRequestModel,
      },
    ...
    for (const { name, exclude, model } of autoModels) {
      ...
      if (name === "User") {
        ...
        resource.create.send.before((req, res, context) => {
          wallet_1.WalletModel.create({ UserId: context.instance.id }).catch(
            (err) => {
              console.log(err);
            }
          );
    ...
    app.get(
      ...
      (0, authenticatedUsers_1.default)()
    );
...
while (
  !expectedModels.every((model) =>
    Object.keys(models_1.sequelize.models).includes(model)
  )
) {
  logger_1.default.info(
    `Entity models ${safe_1.default.bold(
      Object.keys(models_1.sequelize.models).length.toString()
    )} of ${safe_1.default.bold(
      expectedModels.length.toString()
    )} are initialized (${safe_1.default.yellow("WAITING")})`
  );
}
logger_1.default.info(
  `Entity models ${safe_1.default.bold(
    ...
    expectedModels.length.toString()
  )} are initialized (${safe_1.default.green("OK")})`
);
...
errorhandler.title = `${config_1.default.get(
  "application.name"
...
async function start(readyCallback) {
  ...
  process.env.BASE_PATH =
    process.env.BASE_PATH ?? config_1.default.get("server.basePath");
  metricsUpdateLoop = Metrics.updateLoop(); // vuln-code-snippet neutral-line exposedMetricsChallenge
  server.listen(port, () => {
    logger_1.default.info(
      safe_1.default.cyan(
        `Server listening on port ${safe_1.default.bold(`${port}`)}`
      )
    );
    ...
    if (process.env.BASE_PATH !== "") {
      logger_1.default.info(
        safe_1.default.cyan(
          `Server using proxy base path ${safe_1.default.bold(
            `${process.env.BASE_PATH}`
          )} for redirects`
        )
      );
  ...
  void collectDurationPromise(
    ...
    customizeApplication_1.default
  )(); // vuln-code-snippet hide-line
  void collectDurationPromise(
    "customizeEasterEgg",
    customizeEasterEgg_1.default
  )(); // vuln-code-snippet hide-line
}
exports.start = start;
